...
namespace WindowsFormsApp1
{
    public partial class Form6 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            conn.Open();
            string sql = "select * from parking_management";
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("确实要删除该行吗?", "询问", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ...
                //获取此行的值
                string yuangong = this.dataGridView1.Rows[r].Cells["vehicle_ID"].Value.ToString();
                ...
                conn.Open();                        //打了链接
                string sql = "delete from parking_management where vehicle_ID='" + yuangong + "'";
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            conn.Open();//打开连接
                        //往表 Tittle里插入指定内容。
            string str = string.Format("insert into parking_management (vehicle_ID,vehicle_type,vehicle_num,ID,name,tel,fee) values ('{0}','{1}','{2}','{3}','{4}','{5}','{6}')", textBox2.Text, textBox3.Text, textBox4.Text, textBox5.Text, textBox6.Text, textBox7.Text, textBox8.Text);
...
            cn.Open();                                                                            //打开连接的数据库
            MySqlDataAdapter dap = new MySqlDataAdapter("SELECT * FROM parking_management", cn);    //建立SQL语句与数据库的连接
...
            {
                ...
                conn.Open();
                MySqlCommand cmd = new MySqlCommand("select * from parking_management where vehicle_ID like '%" + textBox1.Text + "%'", conn);
                string sqlSel = "select count(*) from parking_management where vehicle_ID ='" + textBox1.Text.Trim() + "'";
                MySqlCommand com = new MySqlCommand(sqlSel, conn);
                if (Convert.ToInt32(com.ExecuteScalar()) <= 0)
                {
                    MessageBox.Show("未找到该车位号！显示失败！");
                    textBox1.Text = " ";
                }
                MySqlDataAdapter sda = new MySqlDataAdapter();
                sda.SelectCommand = cmd;
                DataSet ds = new DataSet();
                sda.Fill(ds, "parking_management");
                dataGridView1.DataSource = ds.Tables["parking_management"];
            }
            if (this.comboBox1.SelectedIndex == 1)//comboBox2下拉框选择笫二项
            {
                conn = new MySqlConnection("server=127.0.0.1;database=mydatabase;uid=root;pwd= ;charset=utf8");
                conn.Open();
                MySqlCommand cmd = new MySqlCommand("select * from parking_management where vehicle_type like '%" + textBox1.Text + "%'", conn);
                string sqlSel = "select count(*) from parking_management where vehicle_type ='" + textBox1.Text.Trim() + "'";
                MySqlCommand com = new MySqlCommand(sqlSel, conn);
                if (Convert.ToInt32(com.ExecuteScalar()) <= 0)
                {
                    MessageBox.Show("未找到该车型！显示失败！");
                    textBox1.Text = " ";
                }
                MySqlDataAdapter sda = new MySqlDataAdapter();
                sda.SelectCommand = cmd;
...
            {
                conn = new MySqlConnection("server=127.0.0.1;database=mydatabase;uid=root;pwd= ;charset=utf8");
                conn.Open();
                MySqlCommand cmd = new MySqlCommand("select * from parking_management where vehicle_num like '%" + textBox1.Text + "%'", conn);
                string sqlSel = "select count(*) from parking_management where vehicle_num ='" + textBox1.Text.Trim() + "'";
                MySqlCommand com = new MySqlCommand(sqlSel, conn);
                if (Convert.ToInt32(com.ExecuteScalar()) <= 0)
                {
                    MessageBox.Show("未找到该车牌号！显示失败！");
                    textBox1.Text = " ";
                }
                MySqlDataAdapter sda = new MySqlDataAdapter();
                sda.SelectCommand = cmd;
...
            {
                conn = new MySqlConnection("server=127.0.0.1;database=mydatabase;uid=root;pwd= ;charset=utf8");
                conn.Open();
                MySqlCommand cmd = new MySqlCommand("select * from parking_management where ID like '%" + textBox1.Text + "%'", conn);
                string sqlSel = "select count(*) from parking_management where ID ='" + textBox1.Text.Trim() + "'";
                MySqlCommand com = new MySqlCommand(sqlSel, conn);
                if (Convert.ToInt32(com.ExecuteScalar()) <= 0)
                {
...
            {
                ...
                string sql = "update parking_management set vehicle_ID='" + textBox10.Text + "' where vehicle_ID='" + textBox9.Text + "'";
                string sqlSel = "select count(*) from parking_management where vehicle_ID ='" + textBox9.Text.Trim() + "'";
                MySqlCommand com = new MySqlCommand(sqlSel, connection);
                if (Convert.ToInt32(com.ExecuteScalar()) <= 0)
                {
                    MessageBox.Show("未找到该车位号！修改失败！");
                    textBox9.Text = "";
                }
                else
...
            {
                ...
                MySqlConnection connection = new MySqlConnection(str);   //连接数据库
                connection.Open();  //打开数据库连接
                string sql = "update parking_management set vehicle_num='" + textBox10.Text + "' where vehicle_ID='" + textBox9.Text + "'";
                string sqlSel = "select count(*) from parking_management where vehicle_ID ='" + textBox9.Text.Trim() + "'";
                MySqlCommand com = new MySqlCommand(sqlSel, connection);
                if (Convert.ToInt32(com.ExecuteScalar()) <= 0)
                {
...
            {
                ...
                MySqlConnection connection = new MySqlConnection(str);   //连接数据库
                connection.Open();  //打开数据库连接
                string sql = "update parking_management set ID='" + textBox10.Text + "' where vehicle_ID'" + textBox9.Text + "'";
                string sqlSel = "select count(*) from parking_management where vehicle_ID ='" + textBox9.Text.Trim() + "'";
                MySqlCommand com = new MySqlCommand(sqlSel, connection);
                if (Convert.ToInt32(com.ExecuteScalar()) <= 0)
                {
...
            {
                ...
                MySqlConnection connection = new MySqlConnection(str);   //连接数据库
                connection.Open();  //打开数据库连接
                string sql = "update parking_management set name='" + textBox10.Text + "' where vehicle_ID='" + textBox9.Text + "'";
                string sqlSel = "select count(*) from parking_management where vehicle_ID ='" + textBox9.Text.Trim() + "'";
                MySqlCommand com = new MySqlCommand(sqlSel, connection);
                if (Convert.ToInt32(com.ExecuteScalar()) <= 0)
                {
...
            {
                ...
                MySqlConnection connection = new MySqlConnection(str);   //连接数据库
                connection.Open();  //打开数据库连接
                string sql = "update parking_management set tel='" + textBox10.Text + "' where vehicle_ID='" + textBox9.Text + "'";
                string sqlSel = "select count(*) from parking_management where vehicle_ID ='" + textBox9.Text.Trim() + "'";
                MySqlCommand com = new MySqlCommand(sqlSel, connection);
                if (Convert.ToInt32(com.ExecuteScalar()) <= 0)
                {
...
            {
                ...
                MySqlConnection connection = new MySqlConnection(str);   //连接数据库
                connection.Open();  //打开数据库连接
                string sql = "update parking_management set fee='" + textBox10.Text + "' where vehicle_ID='" + textBox9.Text + "'";
                string sqlSel = "select count(*) from parking_management where vehicle_ID ='" + textBox9.Text.Trim() + "'";
                MySqlCommand com = new MySqlCommand(sqlSel, connection);
                if (Convert.ToInt32(com.ExecuteScalar()) <= 0)
                {
