...
namespace WindowsFormsApp1
{
    public partial class Form5 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            conn.Open();
            string sql = "select * from mydatabase.repair";
...
            cn.Open();                                                                            //打开连接的数据库
            MySqlDataAdapter dap = new MySqlDataAdapter("SELECT * FROM repair", cn);    //建立SQL语句与数据库的连接
...
            {
                ...
                conn.Open();
                MySqlCommand cmd = new MySqlCommand("select * from repair where house_num like '%" + textBox1.Text + "%'", conn);
                string sqlSel = "select count(*) from repair where house_num ='" + textBox1.Text.Trim() + "'";
...
            {
                conn = new MySqlConnection("server=127.0.0.1;database=mydatabase;uid=root;pwd= ;charset=utf8");
                conn.Open();
                MySqlCommand cmd = new MySqlCommand("select * from repair where name like '%" + textBox1.Text + "%'", conn);
                string sqlSel = "select count(*) from repair where name ='" + textBox1.Text.Trim() + "'";
                MySqlCommand com = new MySqlCommand(sqlSel, conn);
                if (Convert.ToInt32(com.ExecuteScalar()) <= 0)
                {
...
            {
                conn = new MySqlConnection("server=127.0.0.1;database=mydatabase;uid=root;pwd= ;charset=utf8");
                conn.Open();
                MySqlCommand cmd = new MySqlCommand("select * from repair where building_num like '%" + textBox1.Text + "%'", conn);
                string sqlSel = "select count(*) from repair where building_num ='" + textBox1.Text.Trim() + "'";
                MySqlCommand com = new MySqlCommand(sqlSel, conn);
                if (Convert.ToInt32(com.ExecuteScalar()) <= 0)
                {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("确实要删除该行吗?", "询问", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ...
                conn.Open();                        //打了链接
                string sql = "delete from repair where house_num='" + yuangong + "'";
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            conn.Open();//打开连接
                        //往表 Tittle里插入指定内容。
            string str = string.Format("insert into repair (house_num,name,building_num,tel,project,date,attendant) values ('{0}','{1}','{2}','{3}','{4}','{5}','{6}')", textBox2.Text, textBox3.Text, textBox5.Text, textBox4.Text, textBox6.Text, textBox7.Text, textBox8.Text);
...
            {
                ...
                string sql = "update repair set name='" + textBox10.Text + "' where house_num='" + textBox9.Text + "'";
                string sqlSel = "select count(*) from repair where house_num ='" + textBox9.Text.Trim() + "'";
                MySqlCommand com = new MySqlCommand(sqlSel, connection);
                if (Convert.ToInt32(com.ExecuteScalar()) <= 0)
                {
                    MessageBox.Show("未找到该房号！修改失败！");
                    textBox9.Text = " ";
                }
                else
...
            {
                ...
                MySqlConnection connection = new MySqlConnection(str);   //连接数据库
                connection.Open();  //打开数据库连接
                string sql = "update repair set building_num='" + textBox10.Text + "' where house_num='" + textBox9.Text + "'";
                string sqlSel = "select count(*) from repair where house_num ='" + textBox9.Text.Trim() + "'";
                MySqlCommand com = new MySqlCommand(sqlSel, connection);
                if (Convert.ToInt32(com.ExecuteScalar()) <= 0)
                {
...
            {
                ...
                MySqlConnection connection = new MySqlConnection(str);   //连接数据库
                connection.Open();  //打开数据库连接
                string sql = "update repair set tel='" + textBox10.Text + "' where house_num='" + textBox9.Text + "'";
                string sqlSel = "select count(*) from repair where house_num ='" + textBox9.Text.Trim() + "'";
                MySqlCommand com = new MySqlCommand(sqlSel, connection);
                if (Convert.ToInt32(com.ExecuteScalar()) <= 0)
                {
...
            {
                ...
                MySqlConnection connection = new MySqlConnection(str);   //连接数据库
                connection.Open();  //打开数据库连接
                string sql = "update repair set project='" + textBox10.Text + "' where house_num='" + textBox9.Text + "'";
                string sqlSel = "select count(*) from repair where house_num ='" + textBox9.Text.Trim() + "'";
                MySqlCommand com = new MySqlCommand(sqlSel, connection);
                if (Convert.ToInt32(com.ExecuteScalar()) <= 0)
                {
...
            {
                ...
                MySqlConnection connection = new MySqlConnection(str);   //连接数据库
                connection.Open();  //打开数据库连接
                string sql = "update repair set date='" + textBox10.Text + "' where house_num='" + textBox9.Text + "'";
                string sqlSel = "select count(*) from repair where house_num ='" + textBox9.Text.Trim() + "'";
                MySqlCommand com = new MySqlCommand(sqlSel, connection);
                if (Convert.ToInt32(com.ExecuteScalar()) <= 0)
                {
...
            {
                ...
                MySqlConnection connection = new MySqlConnection(str);   //连接数据库
                connection.Open();  //打开数据库连接
                string sql = "update repair set attendant='" + textBox10.Text + "' where house_num='" + textBox9.Text + "'";
                string sqlSel = "select count(*) from repair where house_num ='" + textBox9.Text.Trim() + "'";
                MySqlCommand com = new MySqlCommand(sqlSel, connection);
                if (Convert.ToInt32(com.ExecuteScalar()) <= 0)
                {
